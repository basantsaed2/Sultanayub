...
const BranchCategoryPage = () => {
  ...
  return (
    <div className="flex flex-col h-full bg-gray-50 overflow-hidden">
      {/* Header */}
      <div className="bg-white p-6 shadow-sm border-b shrink-0">
            ...
            <div className="flex bg-gray-100 p-1 rounded-xl">
              <button
                ...
                className={`px-6 py-2 rounded-lg font-medium transition ${
                  activeTab === "products" ? "bg-white shadow text-mainColor" : "text-gray-500"
                }`}
              >
                {t("Products")}
              </button>
              <button
                ...
                className={`px-6 py-2 rounded-lg font-medium transition ${
                  activeTab === "options" ? "bg-white shadow text-mainColor" : "text-gray-500"
                }`}
              >
                {t("Options")}
              </button>
              ...
              {filteredProducts.length === 0 ? (
                ...
                <div className="grid grid-cols-1 gap-4">
                  {filteredProducts.map((product) => (
                    <div key={product.id} className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100 flex items-center gap-6 hover:shadow-md transition">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-gray-500 mt-1 line-clamp-1">{product.description || t("noDescription")}</p>
                      ...
                      <div className="flex flex-col items-end gap-3 shrink-0">
                        ...
                        />
                        <span className={`text-sm font-medium ${product.status === 1 ? 'text-green-600' : 'text-gray-400'}`}>
                          {product.status === 1 ? t("available") : t("unavailable")}
                        </span>
                      </div>
                    </div>
                  ))}

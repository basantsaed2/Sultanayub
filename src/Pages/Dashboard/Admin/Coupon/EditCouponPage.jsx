...
const EditCouponPage = () => {
  ...
  const {
    ...
    loading: loadingAllCoupon,
    data: dataAllCoupon,
  } = useGet({ url: `${apiUrl}/admin/coupon` });
  const {
    refetch: refetchCoupon,
    loading: loadingCoupon,
    data: dataCoupon,
  } = useGet({ url: `${apiUrl}/admin/coupon/item/${couponId}` });
  const { postData, loadingPost, response } = usePost({
    url: `${apiUrl}/admin/coupon/update/${couponId}`,
  });
  ...
  const handleCouponEdit = (e) => {
    ...
    if (!title) {
      auth.toastError(t("please Enter Title"));
      return;
    }
    if (!code) {
      auth.toastError(t("please Enter Code"));
      return;
    }
    if (!startDate) {
    ...
    if (!expireDate) {
      auth.toastError(t("please Enter Expire Date"));
      return;
    }
    if (!valueTypeName) {
      auth.toastError(t("please Select value Type"));
      return;
    }
    if (!discountAmount) {
    ...
    if (!productOptionsName) {
      auth.toastError(t("please Select Product Option"));
      return;
    }
    ...
    if (!couponTypeName) {
      auth.toastError(t("please Select Coupon Type"));
      return;
    }
    if (!minPurchase) {
      auth.toastError(t("please Select Min Purchase"));
      return;
    }
    if (!usageTypeName) {
      auth.toastError(t("please Select Usage Type"));
      return;
    }
    if (!userUsageTypeName) {
      auth.toastError(t("please Select User Usage Type"));
      return;
    }
    if (usageTypeName === "Fixed" && !usageNumber) {
      auth.toastError(t("please Enter Usage Number"));
      return;
    }
    if (userUsageTypeName === "Fixed" && !userUsageNumber) {
      auth.toastError(t("please Enter User Usage Number"));
      return;
    }
    if (maxDiscountStatus === 1 && !maxDiscount) {
      auth.toastError(t("please Enter Max Discount"));
      return;
    }
            ...
            <div className="flex flex-wrap items-center justify-start w-full gap-4 mb-4 sm:flex-col lg:flex-row">
              ...
              <div className="sm:w-full lg:w-[30%] flex flex-col items-start justify-center gap-y-1">
                ...
                <DateInput
                  ...
                  placeholder={t("Coupon Start Date")}
                  maxDate={false}
                />
              </div>
              {/* Coupon Expire Date */}
              <div className="sm:w-full lg:w-[30%] flex flex-col items-start justify-center gap-y-1">
                <span className="text-xl font-TextFontRegular text-thirdColor">
                  {t("Coupon Expire Date")}
                </span>
                <DateInput
                  value={expireDate}
                  onChange={(e) => setExpireDate(e.target.value)}
                  placeholder={t("Coupon Expire Date")}
                  maxDate={false}
                />
              </div>
              {/* Value Types */}
              ...
              <div className="sm:w-full lg:w-[30%] flex flex-col items-start justify-center gap-y-1">
                <span className="text-xl font-TextFontRegular text-thirdColor">
                  {t("Coupon Min Purchase")}
                </span>
                <NumberInput
                  value={minPurchase}
                  onChange={(e) => setMinPurchase(e.target.value)}
                  placeholder={t("Coupon Min Purchase")}
                  maxDate={false}
                />
              </div>
              {/* Coupon Usage Type */}
              ...
              {usageTypeName === "Fixed" && (
                <div className="sm:w-full lg:w-[30%] flex flex-col items-start justify-center gap-y-1">
                  <span className="text-xl font-TextFontRegular text-thirdColor">
                    {t("Coupon Usage Number")}
                  </span>
                  <NumberInput
                    value={usageNumber}
                    onChange={(e) => setUsageNumber(e.target.value)}
                    placeholder={t("Coupon Usage Number")}
                    maxDate={false}
                  />
                </div>
              )}
              ...
              {maxDiscountStatus === 1 && (
                <div className="sm:w-full lg:w-[30%] flex flex-col items-start justify-center gap-y-1">
                  <span className="text-xl font-TextFontRegular text-thirdColor">
                    {t("Coupon Max Discount")}
                  </span>
                  <NumberInput
                    value={maxDiscount}
                    onChange={(e) => setMaxDiscount(e.target.value)}
                    placeholder={t("Coupon Max Discount")}
                    maxDate={false}
                  />
                </div>
              )}

...
const ExtraPage = ({ refetch, setUpdate }) => {
    ...
    const {
        ...
        url: `${apiUrl}/admin/extra_group/group/${groupExtraId}`,
    });
    ...
    const ExtraPerPage = 20; // Limit to 20 Extra per page

    // Calculate total number of pages
    const totalPages = Math.ceil(Extra.length / ExtraPerPage);

    // Get the Extra for the current page
    const currentExtra = Extra.slice(
        (currentPage - 1) * ExtraPerPage,
        currentPage * ExtraPerPage
    );
    ...
    // Fetch Extra when the component mounts or when refetch is called
    useEffect(() => {
        refetchExtra();
    }, [refetchExtra, refetch]); // Empty dependency array to only call refetch once on mount
    ...
    const handleDelete = async (id, name) => {
        const success = await deleteData(
            `${apiUrl}/admin/extra_group/delete/${id}`,
            `${name} Deleted Success.`
        );

        if (success) {
            // Update Extra only if changeState succeeded
                    ...
                    <table className="block w-full overflow-x-scroll sm:min-w-0 scrollPage">
                        ...
                        <tbody className="w-full">
                            {Extra.length === 0 ? (
                                ...
                                currentExtra.map((Extra, index) => (
                                    <tr className="w-full border-b-2" key={index}>
                                        <td className="min-w-[80px] sm:min-w-[50px] sm:w-1/12 lg:w-1/12 py-2 text-center text-thirdColor text-sm sm:text-base lg:text-lg xl:text-xl overflow-hidden">
                                            {(currentPage - 1) * ExtraPerPage + index + 1}
                                        </td>
                                        ...
                                        <td className="px-4 py-3 text-center">
                                            <div className="flex items-center justify-center gap-2">
                                                <Link to={`edit/${Extra.id}`}>
                                                    <EditIcon />
                                                </Link>
                                                <button
                                                    type="button"
                                                    onClick={() => handleOpenDelete(Extra.id)}
                                                >
                                                    <DeleteIcon />
                                                </button>
                                                {openDelete === Extra.id && (
                                                                ...
                                                                <DialogPanel className="relative overflow-hidden text-left transition-all transform bg-white rounded-lg shadow-xl sm:my-8 sm:w-full sm:max-w-lg">
                                                                    ...
                                                                    <div className="px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
                                                                        <button
                                                                            ...
                                                                            onClick={() =>
                                                                                handleDelete(Extra.id, Extra.name)
                                                                            }
                                                                        >
                                                                            {t("Delete")}
                                                                        </button>

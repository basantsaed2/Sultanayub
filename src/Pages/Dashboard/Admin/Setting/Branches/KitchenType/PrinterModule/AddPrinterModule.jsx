...
const AddPrinterModule = () => {
    ...
    const { postData, loadingPost, response } = usePost({
        url: `${apiUrl}/admin/kitchen_printer/add`,
    });
    const { refetch, loading, data } = useGet({ url: `${apiUrl}/admin/kitchen_printer/lists` });
    ...
    // Format incoming data for React Select
    useEffect(() => {
        if (data) {
            if (data.group_products) {
                const groups = data.group_products.map(g => ({
                    value: g.id,
                    label: g.name
                }));
            ...
            if (data.modules) {
                const mods = data.modules.map(m => ({
                    value: m,
                    label: t(m) // Optional: translate module names
                }));
    ...
    const handleAdd = (e) => {
        ...
        postData(formData, t("PrinterModuleAddedSuccess"));
    };
            ...
            {loadingPost || loading ? (
                ...
                <section className="flex flex-col mb-20">
                    <div className="flex items-center gap-2">
                        ...
                        <TitlePage text={t("AddNewPrinterModule")} />
                    </div>
...
export default AddPrinterModule;
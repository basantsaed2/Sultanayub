...
const AddBannerSection = ({ update, setUpdate }) => {
  ...
  const handleBranchAdd = (e) => {
    ...
    if (branchName.length === 0) {
      auth.toastError('please Enter Branch Names')
      return;
    }
    ...
    branchName.forEach((name, index) => {
      ...
      formData.append(`branch_names[${index}][tranlation_id]`, name.tranlation_id);
      formData.append(`branch_names[${index}][branch_name]`, name.branch_name);
      formData.append(`branch_names[${index}][tranlation_name]`, name.tranlation_name);
    });

...
const GroupPage = ({ refetch, setUpdate }) => {
    ...
    const GroupPerPage = 20; // Limit to 20 Group per page

    // Calculate total number of pages
    const totalPages = Math.ceil(Group.length / GroupPerPage);

    // Get the Group for the current page
    const currentGroup = Group.slice(
    ...
    // Fetch Group when the component mounts or when refetch is called
    useEffect(() => {
        refetchGroup();
    }, [refetchGroup, refetch]); // Empty dependency array to only call refetch once on mount
    ...
    const handleDelete = async (id, name) => {
        const success = await deleteData(
            `${apiUrl}/admin/group/delete/${id}`,
            `${name} Deleted Success.`
        );

        if (success) {
            // Update Group only if changeState succeeded
                                ...
                                currentGroup.map((group, index) => (
                                    <tr className="w-full border-b-2" key={index}>
                                        <td className="min-w-[80px] sm:min-w-[50px] sm:w-1/12 lg:w-1/12 py-2 text-center text-thirdColor text-sm sm:text-base lg:text-lg xl:text-xl overflow-hidden">
                                            {(currentPage - 1) * GroupPerPage + index + 1}
                                        </td>
                                        <td className="min-w-[150px] sm:min-w-[100px] sm:w-2/12 lg:w-2/12 py-2 text-center text-thirdColor text-sm sm:text-base lg:text-lg xl:text-xl overflow-hidden">
                                            {group?.name || "-"}
                                        </td>
                                        <td className="min-w-[150px] sm:min-w-[100px] sm:w-2/12 lg:w-2/12 py-2 text-center text-blue-500 underline text-sm sm:text-base lg:text-lg xl:text-xl overflow-hidden">
                                            <Link to={`view/${group.id}`}>
                                                View
                                            </Link>
                                        </td>
                                        <td className="min-w-[150px] sm:min-w-[100px] sm:w-2/12 lg:w-2/12 py-2 text-center text-thirdColor text-sm sm:text-base lg:text-lg xl:text-xl overflow-hidden">

...
const CustomersDue = () => {
        ...
        if (numericValue < totalAllocated) {
            ...
            setPaymentData(prev => ({
                ...
                financials: prev.financials.map(financial => ({
                    ...financial,
                    amount: financial.amount ? (parseFloat(financial.amount) * adjustmentRatio).toFixed(2) : ""
                }))
            }));
        }
    };
    ...
    const handleFinancialAmountChange = (index, value) => {
        ...
        const currentRowAmount = parseFloat(paymentData.financials[index]?.amount || 0);
        ...
        if (amountChange > remainingPaymentAmount && remainingPaymentAmount >= 0) {
            setErrors(prev => ({
                ...prev,
                [`financial_${index}`]: `Only ${remainingPaymentAmount.toFixed(2)} EGP available in payment`
            }));
            return;
        }
    ...
    const autoDistributeRemaining = () => {
        const remainingPaymentAmount = getRemainingPaymentAmount();
        const emptyFinancials = paymentData.financials.filter(f => !f.amount || f.amount === "0" || f.amount === "");
        
        if (emptyFinancials.length > 0 && remainingPaymentAmount > 0) {
            const distributedAmount = (remainingPaymentAmount / emptyFinancials.length).toFixed(2);
            
            setPaymentData(prev => ({
                ...prev,
                financials: prev.financials.map(financial => 
                    (!financial.amount || financial.amount === "0" || financial.amount === "") 
    ...
    const handlePaymentSubmit = async () => {
        ...
        if (totalAllocated !== paymentAmount) {
            setErrors(prev => ({
                ...prev,
                financials: "Total payment methods amount must equal the payment amount"
            }));
            return;
        }

        // Filter out empty financial entries
        const validFinancials = paymentData.financials.filter(f => f.id && f.amount && parseFloat(f.amount) > 0);
    ...
    const totalAllocated = getTotalAllocatedAmount();
    const isFullyAllocated = remainingPaymentAmount === 0 && totalAllocated === parseFloat(paymentData.amount || 0);
    const isPartialPayment = parseFloat(paymentData.amount || 0) < (selectedCustomer?.due || 0);
                        ...
                        <div className="fixed inset-0 flex items-center justify-center p-4">
                            <DialogPanel className="flex flex-col w-full max-w-2xl bg-white rounded-xl max-h-[90vh]">
                                ...
                                <div className="flex-shrink-0 flex items-center justify-between p-6 border-b border-gray-200">
                                    <h2 className="text-xl font-bold text-gray-900">Pay Due Amount</h2>
                                    <button 
                                    ...
                                    <div className="p-6 space-y-6">
                                        {selectedCustomer && (
                                            <div className="p-4 bg-blue-50 border border-blue-200 rounded-lg">
                                                <h3 className="font-semibold text-gray-800">Customer: {selectedCustomer.name}</h3>
                                                <div className="flex items-center justify-between mt-2">
                                                    <span className="text-lg font-bold text-red-600">
                                                        Total Due: {selectedCustomer.due} EGP
                                                    </span>
                                                ...
                                                {isPartialPayment && (
                                                    <p className="text-sm text-orange-600 mt-1 font-medium">
                                                        Remaining Due After Payment: {remainingDueAmount.toFixed(2)} EGP
                                                    </p>
                                                )}
                                            </div>
                                        )}
                                        ...
                                        <div className="space-y-6">
                                            <div>
                                                ...
                                                <p className="text-sm text-gray-500 mt-1">
                                                    You can pay any amount up to {selectedCustomer?.due} EGP
                                                </p>
                                            </div>
                                            ...
                                            <div className="border-t pt-4">
                                                <div className="flex items-center justify-between mb-4">
                                                    <div>
                                                        <label className="block font-medium text-gray-700">Payment Methods</label>
                                                        <p className="text-sm text-gray-500 mt-1">
                                                            Allocated: <span className="font-semibold">{totalAllocated.toFixed(2)} EGP</span> 
                                                            {remainingPaymentAmount > 0 && (
                                                                <span className="ml-2">
                                                                    • Available: <span className="font-semibold text-green-600">{remainingPaymentAmount.toFixed(2)} EGP</span>
                                                                </span>
                                                            )}
                                                        </p>
                                                    </div>
                                                ...
                                                <div className="space-y-3">
                                                    {paymentData.financials.map((financial, index) => (
                                                        <div key={index} className="flex gap-3 p-3 border border-gray-200 rounded-lg bg-white">
                                                            <div className="flex-1">
                                                                <select
                                                                    value={financial.id}
                                                                    onChange={(e) => updateFinancialData(index, 'id', e.target.value)}
                                                                    className="w-full p-2 border border-gray-300 rounded focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                                                                >
                                                                    <option value="">Select Payment Method</option>
                                                                    {dataCustomerDue?.financials?.map(method => (
                                                            ...
                                                            <div className="flex-1">
                                                                <input
                                                                    ...
                                                                    className={`w-full p-2 border rounded focus:ring-2 focus:ring-blue-500 focus:border-transparent ${
                                                                        errors[`financial_${index}`] ? 'border-red-500' : 'border-gray-300'
                                                                    }`}
                                ...
                                {/* Fixed Footer */}
                                <div className="flex-shrink-0 flex justify-end gap-3 p-6 border-t border-gray-200 bg-gray-50 rounded-b-xl">
                                    <button
                                        onClick={() => setIsPaymentModalOpen(false)}
                                    ...
                                    <button
                                        ...
                                        className="px-6 py-2 text-white bg-red-500 rounded-lg hover:bg-red-600 disabled:bg-gray-400 disabled:cursor-not-allowed transition-colors"
                                    >
                                        {loadingPost ? (

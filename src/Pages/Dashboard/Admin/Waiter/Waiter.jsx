...
const Waiter = ({ refetch, setUpdate }) => {
  const apiUrl = import.meta.env.VITE_API_BASE_URL;
  const {
    refetch: refetchWaiter,
    loading: loadingWaiter,
    data: dataWaiter,
  } = useGet({
    url: `${apiUrl}/admin/waiter`,
  });
  ...
  const waitersPerPage = 20; // Limit to 20 waiters per page

  // Calculate total number of pages
  const totalPages = Math.ceil(waiters.length / waitersPerPage);

  // Get the waiters for the current page
  const currentWaiter = waiters.slice(
    (currentPage - 1) * waitersPerPage,
    currentPage * waitersPerPage
  );
  ...
  // Fetch Waiter when the component mounts or when refetch is called
  useEffect(() => {
    refetchWaiter();
  }, [refetchWaiter, refetch]); // Empty dependency array to only call refetch once on mount


  // Update Waiter when `data` changes
  useEffect(() => {
    if (dataWaiter && dataWaiter.waiter) {
      setWaiter(dataWaiter.waiter);
    }
  }, [dataWaiter]); // Only run this effect when `data` changes
  ...
  const handleDelete = async (id, name) => {
    const success = await deleteData(
      `${apiUrl}/admin/waiter/delete/${id}`,
      `${name} Deleted Success.`
    );

    if (success) {
      // Update Waiter only if changeState succeeded
                  ...
                  (
                    ...
                    <tr className="w-full border-b-2" key={index}>
                      <td className="min-w-[80px] sm:min-w-[50px] sm:w-1/12 lg:w-1/12 py-2 text-center text-thirdColor text-sm sm:text-base lg:text-lg xl:text-xl overflow-hidden">
                        {(currentPage - 1) * waitersPerPage + index + 1}
                      </td>
                      ...
                      <td className="min-w-[150px] sm:min-w-[100px] sm:w-2/12 lg:w-2/12 py-2 text-center text-thirdColor text-sm sm:text-base lg:text-lg xl:text-xl overflow-hidden">
                        <span
                          ...
                          onClick={() => handleOpenLocations(waiter.locations)}
                        >
                          {t("View")}
                        </span>
                      </td>
                                ...
                                <div className="flex flex-wrap items-center justify-center w-full gap-4 px-4 my-4 sm:p-6 sm:pb-4">
                                  {openLocation.length === 0 ? (
                                    <div className="w-full my-4 text-lg text-center text-gray-500 font-TextFontSemiBold">
                                      {t("No location available for this waiter order")}
                                    </div>
                                  ) : (
                                    openLocation.map(
                                      (location, index) => {
                      ...
                      <td className="px-4 py-3 text-center">
                        <div className="flex items-center justify-center gap-2">
                          <Link to={`edit/${waiter.id}`}>
                            <EditIcon />
                          </Link>
                          <button
                            type="button"
                            onClick={() => handleOpenDelete(waiter.id)}
                          >
                            <DeleteIcon />
                          </button>
                          {openDelete === waiter.id && (
                            <Dialog
                              open={true}
                                    ...
                                    <div className="flex flex-col items-center justify-center px-4 pt-5 pb-4 bg-white sm:p-6 sm:pb-4">
                                      ...
                                      <div className="flex items-center">
                                        <div className="mt-2 text-center">
                                          {t("You will delete")} {waiter.user_name}
                                        </div>
                                      </div>
                                    </div>
                                    <div className="px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
